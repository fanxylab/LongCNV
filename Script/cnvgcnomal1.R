...
plotCNV <-function(corseg, out, heig=4){
  ...
  corseg$SID = gsub('PDACDNA1_', 'PCC_', corseg$SID)
  if (length(unique(corseg$SID)) >1){
    leves <- c('PCC_BC2','PCC_BC3','PCC_BC7', 'PCC_BC1','PCC_BC4', 'PCC_BC5', 'PCC_BC6','PCC_BC9','PCC_BC10','PCC_BC11','PCC_BC12')#as.character(sort(unique(corseg$SID)))
    corseg$SID <- factor(corseg$SID, levels=leves)
  }
  g <- ggplot(corseg, aes(x=start, y=logcopy)) + 
      geom_point(aes(color = logcopy),size=0.3 ,stroke = 0) +
  ...
  g <- g + theme(text= element_text(color = "black",size=13),
                 legend.text=element_text(color = "black",size = 13),
                 panel.spacing.x=unit(0.1, "lines"),
                 panel.background=element_rect(color="grey",fill=NA,size=0.05,linetype ='dotted'),
  ...
  ggsave(out, g, width=11, height = 9)
}

plotCNV0 <-function(corseg, out, heig=4){
  library(ggplot2)
  library(ggnewscale)
  corseg$SID = gsub('PDACDNA1_', '', corseg$SID)
  corseg['meanCN'] = 2^corseg['meanlog2CN']
  levec <- as.character(sort(as.numeric(unique(corseg$chrom))))
  corseg$chrom <- factor(corseg$chrom, levels=levec)
  if (length(unique(corseg$SID)) >1){
    leves <- c('BC2','BC3','BC7', 'BC1','BC4', 'BC5', 'BC6','BC9','BC10','BC11','BC12')
    corseg$SID <- factor(corseg$SID, levels=leves)
  }
  g <- ggplot(corseg, aes(x=start, y=copy)) + 
      #geom_point(size=0.001,  aes(colour = logcopy) ) +
      ylim(0,8)+
      scale_colour_viridis_c("meamlogcopy", begin=1, end=0.4) + 
      new_scale_colour() + 
      #geom_step( aes(x = start, y = meanlog2CN, color=meanlog2CN),size=0.5, na.rm = TRUE) +
      geom_segment(aes(sstart, meanCN, xend=send, yend=meanCN, color=CNtype), size = 0.65) 
#scale_colour_gradientn(colours=c("blue", "red")) 
        #scale_y_continuous(breaks=c(-5,0,5))
  #scale_colour_gradientn(colours=c("green", "red"))
  #geom_segment(aes(sstart, meanlog2CN, xend=send, yend=meanlog2CN, color=meanlog2CN), size = 0.65) +
  g <- g + labs(x="", y="CNV_log2count profile\n")
  g <- g + facet_grid(SID~chrom, space="free", scales="free", margins=FALSE)
  g <- g + theme(text= element_text(color = "black",size=15),
                 legend.text=element_text(color = "black",size = 15),
                 panel.spacing.x=unit(0, "lines"),
                 #strip.background = element_rect(color = "black", size = 1),
                 #strip.background =element_rect(fill="white"),
                 panel.background = element_rect(color = "black", fill = NA, size = 0.1),
                 panel.border = element_blank(), #element_rect(color = "black", linetype ='dotted', fill = NA, size=0.1),
...
plotCNVCN <-function(corseg, out, heig=4){
  library(ggplot2)
  library(ggnewscale)
  corseg$SID = gsub('PDACDNA1_', '', corseg$SID)
  corseg['meanCN'] = 2^corseg['meanlog2CN']
  ...
  corseg$CNrange <- NA
  corseg$CNrange[corseg$meanCN<=1 | is.na(corseg$logcopy)] <- '<=1'
  corseg$CNrange[corseg$meanCN>1  & corseg$meanCN<=2 &
                                   !is.na(corseg$logcopy)] <- '1-2'
  corseg$CNrange[corseg$meanCN>2  & corseg$meanCN<=4 &
                                    !is.na(corseg$logcopy)] <- '2-4'
  corseg$CNrange[corseg$meanCN>=4  & corseg$meanCN<10 &
                                    !is.na(corseg$logcopy)] <- '4-10'
  corseg$CNrange[corseg$meanCN>=10 & !is.na(corseg$logcopy)] <- '>=10'

  g <- ggplot(corseg, aes(x=start, y=copy)) + 
      geom_point(size=0.001,  aes(colour = copy) ) +
      #scale_colour_viridis_c("meancopy", begin=1, end=0.4) + 
      scale_fill_gradientn(colours=c("green", "yellow"),limits=c(0, 8)) + 
      new_scale_colour() + 
      #geom_step( aes(x = start, y = meanlog2CN, color=meanlog2CN),size=0.5, na.rm = TRUE) +
      geom_segment(aes(sstart, meanCN, xend=send, yend=meanCN, color=CNrange), size = 0.65)  
#scale_colour_gradientn(colours=c("blue", "red")) 
        #scale_y_continuous(breaks=c(-5,0,5))
  #scale_colour_gradientn(colours=c("green", "red"))
  #geom_segment(aes(sstart, meanlog2CN, xend=send, yend=meanlog2CN, color=meanlog2CN), size = 0.65) +
  g <- g + ylim(0,8)+ labs(x="", y="CNV count profile\n")
  g <- g + facet_grid(SID~chrom, space="free", scales="free", margins=FALSE)
  g <- g + theme(text= element_text(color = "black",size=15),
                 legend.text=element_text(color = "black",size = 15),
                 panel.spacing.x=unit(0, "lines"),
